...
hist = hist[hist.index >= "2023-01-01"]

# Salva i dati filtrati in un file CSV
hist.to_csv('nasdaq_historical_data.csv')


# Supponiamo di avere un file CSV con i dati storici
data = pd.read_csv('nasdaq_historical_data.csv')
...
n_qubits = 6  # Numero di qubit (dipende da come encodi i tuoi dati)
dev = qml.device('default.qubit', wires=n_qubits)
...
plt.savefig('nasdaq_previsioni.png')
